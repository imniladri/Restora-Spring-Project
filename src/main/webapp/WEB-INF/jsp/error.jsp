...
<head>
    ...
    <link rel="stylesheet" href="<c:url value='/resources/css/pages/error.css'/>" />
...
<body>
    ...
    <section id="error">
        <div class="error">
            ...
            <img src="<c:url value='/resources/assets/img/error.png'/>" alt="Status Image" />
            <h2>
                <c:choose>
                    <c:when test="${errorHead!=null}">${errorHead}</c:when>
                    <c:otherwise>Error 404!</c:otherwise>
                </c:choose>
            </h2>
            <p>
                <c:choose>
                    <c:when test="${errorPara!=null}">${errorPara}</c:when>
                    <c:otherwise>Kindly Go Back! And Try Once!</c:otherwise>
                </c:choose>
            </p>
            
            <script>
                console.log(`${errorExceptionName} = ${errorExceptionMsg}`);
            </script>

            <a onclick="history.back()" href="javascript:void(0)" class="btn">
                Go Back
            </a>
